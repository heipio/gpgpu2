...
module csr_regfile #(
  ...
  localparam logic [AXI_ADDR_WIDTH-1:0] CSR_FAULT_META_ADDR = 'h0014;
  localparam logic [AXI_ADDR_WIDTH-1:0] CSR_CAP_MAGIC_ADDR = 'h0020;
  localparam logic [AXI_ADDR_WIDTH-1:0] CSR_CAP_VERSION_ADDR = 'h0024;
  localparam logic [AXI_ADDR_WIDTH-1:0] CSR_CAP_GEOMETRY_ADDR = 'h0028;
  ...
  localparam logic [31:0] CAP_VERSION = 32'h0001_0000;
  localparam logic [31:0] CAP_GEOMETRY = {8'd4, 8'd4, 8'd8, 8'd32};
  localparam logic [31:0] CAP_FEATURES = 32'h0000_07ff;
  localparam logic [31:0] CAP_LIMITS = {8'd0, 8'd8, 8'd8, 8'd255};
  localparam logic [31:0] CAP_MEMORY = {8'd0, 8'd32, 8'd4, 8'd32};
  ...
  wire read_is_fault_meta = (s_axil_araddr == CSR_FAULT_META_ADDR);
  wire read_is_cap_magic = (s_axil_araddr == CSR_CAP_MAGIC_ADDR);
  wire read_is_cap_version = (s_axil_araddr == CSR_CAP_VERSION_ADDR);
  wire read_is_cap_geometry = (s_axil_araddr == CSR_CAP_GEOMETRY_ADDR);
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (!s_axil_rvalid && s_axil_arvalid) begin
        ...
        end else if (read_is_fault_meta) begin
          s_axil_rdata <= fault_meta_q;
        end else if (read_is_cap_magic) begin
          s_axil_rdata <= CAP_MAGIC;
        end else if (read_is_cap_version) begin
          s_axil_rdata <= CAP_VERSION;
        end else if (read_is_cap_geometry) begin
          s_axil_rdata <= CAP_GEOMETRY;
        end else if (read_is_cap_features) begin
          s_axil_rdata <= CAP_FEATURES;
        end else if (read_is_cap_limits) begin
