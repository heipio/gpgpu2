...
module prf_top #(
  ...
  logic [7:0] pred_mem [0:NUM_WARPS-1][0:7][0:ISSUE_BEATS-1];
  ...
  function automatic logic [7:0] read_pred_mask(
    ...
    begin
      if (pred == PRED_PT) begin
        ...
        read_pred_mask = pred_mem[read_warp_i[WARP_BITS-1:0]][pred[2:0]][beat];
      end else begin
        read_pred_mask = ~pred_mem[read_warp_i[WARP_BITS-1:0]][pred[2:0]][beat];
      end
    end
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      for (int warp_idx = 0; warp_idx < NUM_WARPS; warp_idx = warp_idx + 1) begin
        for (pred_idx = 0; pred_idx < 8; pred_idx = pred_idx + 1) begin
          for (beat_idx = 0; beat_idx < ISSUE_BEATS; beat_idx = beat_idx + 1) begin
            pred_mem[warp_idx][pred_idx][beat_idx] <= 8'h00;
          end
        end
      end
    end else if (write_valid_i) begin
      pred_mem[write_warp_i[WARP_BITS-1:0]][write_pred_i][write_beat_i] <=
          (pred_mem[write_warp_i[WARP_BITS-1:0]][write_pred_i][write_beat_i] & ~write_mask_i) |
